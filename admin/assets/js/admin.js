...
/* ============================================
   11. رفع الصور على ImageKit CDN
   الـ Private Key محفوظ في Supabase Edge Function
   المتصفح يستخدم Public Key + توقيع مؤقت فقط
   ============================================ */
const _IK_PUBLIC_KEY  = 'public_UX9ff6muz2Vdpj6YH6hB2xxBi/Y=';
const _IK_ENDPOINT    = 'https://ik.imagekit.io/autism';
const _IK_UPLOAD_URL  = 'https://upload.imagekit.io/api/v1/files/upload';
const _IK_AUTH_FN     = `${_ADMIN_SB_URL}/functions/v1/imagekit-auth`;

/**
 * يطلب توقيع مؤقت من Edge Function ثم يرفع الملف مباشرةً على ImageKit
 * @param {File} file - الملف المراد رفعه
 * @param {string} folder - مجلد ImageKit (اختياري)
 * @returns {Promise<string>} رابط URL الكامل للملف على ImageKit
 */
async function uploadToImageKit(file, folder = 'images') {
  await _sbAdminReady;
  if (!_sbAdmin) throw new Error('Supabase غير متاح');

  /* 1. الحصول على توقيع مؤقت من Edge Function */
  const { data: { session } } = await _sbAdmin.auth.getSession();
  if (!session) throw new Error('يجب تسجيل الدخول أولاً');

  const authRes = await fetch(_IK_AUTH_FN, {
    method: 'POST',
    headers: { Authorization: `Bearer ${session.access_token}` }
  });
  if (!authRes.ok) {
    const errText = await authRes.text();
    throw new Error(`فشل توقيع ImageKit: ${errText}`);
  }
  const { token, expire, signature } = await authRes.json();

  /* 2. رفع الملف مباشرةً على ImageKit */
  const safeName = `${Date.now()}-${file.name.replace(/[^\w.\-]/g, '_')}`;
  const form = new FormData();
  form.append('file',            file);
  form.append('fileName',        safeName);
  form.append('folder',          `/bsu/${folder}`);
  form.append('publicKey',       _IK_PUBLIC_KEY);
  form.append('signature',       signature);
  form.append('expire',          String(expire));
  form.append('token',           token);
  form.append('useUniqueFileName', 'false');

  const uploadRes = await fetch(_IK_UPLOAD_URL, { method: 'POST', body: form });
  const result    = await uploadRes.json();

  if (!uploadRes.ok) throw new Error(result.message || 'فشل الرفع على ImageKit');

  return result.url; // e.g. https://ik.imagekit.io/autism/bsu/news/image.jpg
}

/**
 * ربط حدث اختيار صورة بالرفع على ImageKit
 * الـ callback يستقبل URL بدلاً من base64
 * @param {HTMLInputElement} inputEl  - حقل <input type="file">
 * @param {HTMLImageElement|null} previewEl - عنصر المعاينة (اختياري)
 * @param {function(string,string)} callback - يُستدعى بـ (url, fileName)
 * @param {string} folder - مجلد ImageKit
 */
function handleImageUpload(inputEl, previewEl, callback, folder = 'images') {
  inputEl.addEventListener('change', async function () {
    ...
    /* معاينة فورية محلية قبل الرفع */
    const localUrl = URL.createObjectURL(file);
    if (previewEl) { previewEl.src = localUrl; previewEl.classList.add('show'); }

    /* تعطيل زر الحفظ أثناء الرفع */
    const modal   = inputEl.closest('.modal, form') || document.body;
    const saveBtn = modal.querySelector('[type=submit], .btn-save, .btn-primary');
    if (saveBtn) { saveBtn.disabled = true; saveBtn._ikBusy = true; }

    showToast('جاري رفع الصورة على ImageKit…', 'warning');
    ...
    try {
      ...
      showToast('فشل رفع الصورة — ' + (err.message || err), 'danger');
      /* callback بـ null حتى لا يُحفَظ base64 في DB */
      if (callback) callback(null, file.name);
    } finally {
      if (saveBtn?._ikBusy) { saveBtn.disabled = false; delete saveBtn._ikBusy; }
    }
  });
}

/* ============================================
   12. رفع ملفات PDF على ImageKit
   ============================================ */
function handleFileUpload(inputEl, callback, folder = 'projects') {
  inputEl.addEventListener('change', async function () {
    const file = this.files[0];
    if (!file) return;
    ...
    showToast('جاري رفع الملف على ImageKit…', 'warning');
    ...
    try {
      ...
      console.error('ImageKit PDF error:', err);
      showToast('فشل رفع الملف — ' + (err.message || err), 'danger');
      if (callback) callback(null, file.name);
    } finally {
      if (saveBtn?._ikBusy) { saveBtn.disabled = false; delete saveBtn._ikBusy; }
    }
