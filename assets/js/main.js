...
async function _getFromSupabase(db, key) {
  switch (key) {
    case 'news': {
      const { data: news, error: e1 } = await db.from('news').select('*')
        .eq('published', true).order('date', { ascending: false });
      if (e1) throw e1;
      const rows = news || [];
      const categories = [...new Set(rows.map(r => r.category).filter(Boolean))].sort();
      return { news: rows, categories };
    }
    ...
    case 'staff': {
      ...
      return {
        ...
        professor_present:     rows.filter(r => r.role === 'professor-present'),
        ...
        lecturer_assistants:   rows.filter(r => r.role === 'lecturer-assistant'),
...
function _buildHeader(activePage, settings) {
  ...
  const navItems = [
    ...
    { id: 'about',    label: 'عن القسم',            href: 'about.html' },
  ...
  const facName     = settings.faculty_name || 'قسم اضطراب التوحد - كلية علوم ذوي الاحتياجات الخاصة';
